 ...
 * `let serverRef` singleton, set synchronously per request via
 * `setServerRef` and read back later via `getServerRef` — including after
 * `await` gaps inside async tool handlers (e.g. after awaiting an
 * Alternative Payments API call, before sending an elicitation/confirmation
