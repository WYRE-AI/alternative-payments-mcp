 ...
 * (`elicitation/confirm.ts`) was stored in a module-level `let serverRef`
 * singleton in `utils/server-ref.ts` (`setServerRef` / `getServerRef`), set
 * synchronously per request and read back later — including after `await`
 * gaps inside async tool handlers (e.g. after awaiting an Alternative
 * Payments API call, before sending an elicitation/confirmation prompt back
 * through "the" server).
 *
...
import { elicitConfirmation } from '../elicitation/confirm.js';
...
describe('server-ref cross-tenant isolation', () => {
  ...
  it(
    ...
    async () => {
      ...
      // (simulating an in-flight Alternative Payments API call inside a
      ...
      expect(serverA.elicitInput).toHaveBeenCalledWith(
        expect.objectContaining({
          message: expect.stringContaining("Confirm tenant A's sensitive action?"),
        }),
      );
      ...
      expect(serverB.elicitInput).toHaveBeenCalledWith(
        expect.objectContaining({
          message: expect.stringContaining("Confirm tenant B's sensitive action?"),
        }),
      );
      ...
      for (const call of serverA.elicitInput.mock.calls) {
        expect(call[0].message).not.toContain("Confirm tenant B's sensitive action?");
      }
      for (const call of serverB.elicitInput.mock.calls) {
        expect(call[0].message).not.toContain("Confirm tenant A's sensitive action?");
      }
    },
  );
